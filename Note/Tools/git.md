...
> Question：为何需要暂存区？为什么git add不直接将文件提交到版本库？
...
git diff [fileName] #文件修改后(未暂存),查看修改前后区别
git log [fileName][--pretty=oneline] #查看修改日志（历史记录）[简化输出信息]
git reflog  #查看历史命令（可结合版本回退，回退到指定版本）
#输出
...
在工作区的所有文件都已被git所追踪的条件下,当在工作区中删除文件后, 使用 `git status` 命令查看工作区状态 , git会提示哪些文件被删除了
